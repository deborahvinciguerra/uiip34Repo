...
insert into persone
	value	(1,'eugenio','trodella','24 marzo 1988',444444444,13),
			(2,'angelo', 'manganiello', '8 novembre 1986',888888888,3),
            (3,'carla','ventre', '19 giugno,1992',015555553,10),
            (4,'natalia','albanese','25 dicembre 1995',100555506,13),
            (5,'andrea','franza','23 luglio 1991',555555551,7),
            (6,'daniele','Dangela','12 ottobre 1991',895555552,17),
            (7,'gianluca','cocco','16 ottobre 1991',155555565,1),
            (8,' francesca','farinaccio','18 settembre 1993',987555573,11);
            




insert into persone  values (9,' gianluigi','leone','18 settembre 1993',987555573,7);

select r.name, r.cognome, r.dataNascita, s.name from rubrica as r
inner join smartphone as s on s.id=r.smartphone;

select s.name, s.cpu, b.name from smartphone as s
inner join brand as b on b.id=s.brand;

update rubrica set dataNascita='24 settembre 1988'
	where id=9;
    
update smartphone set weight=500
	where id=1;
